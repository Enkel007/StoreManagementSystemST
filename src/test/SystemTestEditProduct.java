...
public class SystemTestEditProduct extends ApplicationTest {
    Button loginButton;
    ...
    @Test
    public void should_navigate_to_edit_product_page() throws SQLException {
        ...
        Button editProductButton = lookup(".button.primary.xs").nth(0).queryAs(Button.class);
        clickOn(editProductButton);

        // Wait for the scene to update
        WaitForAsyncUtils.waitForFxEvents();

        // Verify the scene has changed to the edit product page
        assertThat(Optional.ofNullable(lookup("#formEditProductView").query())).isNotNull();
