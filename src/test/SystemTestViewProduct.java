...
public class SystemTestViewProduct extends ApplicationTest {
    Button loginButton;
    ...
    @Test
    public void should_navigate_to_view_product_page() throws SQLException {
        ...
        // Find and click the view button for the first product
        viewProductButton = lookup(".button.info.xs").nth(0).queryAs(Button.class);
        clickOn(viewProductButton);

        // Wait for the scene to update
        WaitForAsyncUtils.waitForFxEvents();

        // Verify the scene has changed to the view product page
        assertThat(Optional.ofNullable(lookup("#formViewProductView").query())).isNotNull();
